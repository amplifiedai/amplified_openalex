defmodule Amplified.OpenAlex.Keywords do
  @moduledoc """
  Provides API endpoints related to keywords
  """

  @default_client Amplified.OpenAlex.Client

  @doc """
  Get a single keyword

  Retrieve a single keyword by OpenAlex ID.

  ## Options

    * `select`: Comma-separated list of fields to return. Reduces response size. Example: `select=id,display_name,cited_by_count`
    * `api_key`: Your OpenAlex API key (required). Get a free key at https://openalex.org/settings/api

  """
  @spec get_keyword(id :: String.t(), opts :: keyword) ::
          {:ok, Amplified.OpenAlex.Keyword.t()} | {:error, Amplified.OpenAlex.Error.t()}
  def get_keyword(id, opts \\ []) do
    client = opts[:client] || @default_client
    query = Keyword.take(opts, [:api_key, :select])

    client.request(%{
      args: [id: id],
      call: {Amplified.OpenAlex.Keywords, :get_keyword},
      url: "/keywords/#{id}",
      method: :get,
      query: query,
      response: [
        {200, {Amplified.OpenAlex.Keyword, :t}},
        {404, {Amplified.OpenAlex.Error, :t}},
        {429, {Amplified.OpenAlex.Error, :t}}
      ],
      opts: opts
    })
  end

  @doc """
  List keywords

  Get a list of keywords with optional filtering, searching, sorting, and pagination.

  ## Options

    * `filter`: Filter keywords. Examples: `works_count:>100`, `cited_by_count:>1000` See [all Keyword fields](/api-reference/keywords#available-filter-sort-and-group_by-fields).
    * `sort`: Sort by: `works_count`, `cited_by_count`. Prefix with `-` for descending. See [all Keyword fields](/api-reference/keywords#available-filter-sort-and-group_by-fields).
    * `search`: Full-text search across titles, abstracts, and other text fields. Example: `search=machine learning`
    * `per_page`: Number of results per page (1-100, default 25)
    * `page`: Page number for pagination. Use cursor for deep pagination beyond 10,000 results.
    * `cursor`: Cursor for deep pagination. Use `cursor=*` to start, then use the `next_cursor` from the response.
    * `sample`: Return a random sample of N results (max 10,000). Cannot be used with sort or page.
    * `select`: Comma-separated list of fields to return. Reduces response size. Example: `select=id,display_name,cited_by_count`
    * `group_by`: Group results by a field and return counts. Available fields vary by entity type. See [all Keyword fields](/api-reference/keywords#available-filter-sort-and-group_by-fields).
    * `api_key`: Your OpenAlex API key (required). Get a free key at https://openalex.org/settings/api

  """
  @spec list_keywords(opts :: keyword) ::
          {:ok, Amplified.OpenAlex.KeywordsListResponse.t()}
          | {:error, Amplified.OpenAlex.Error.t()}
  def list_keywords(opts \\ []) do
    client = opts[:client] || @default_client

    query =
      Keyword.take(opts, [
        :api_key,
        :cursor,
        :filter,
        :group_by,
        :page,
        :per_page,
        :sample,
        :search,
        :select,
        :sort
      ])

    client.request(%{
      args: [],
      call: {Amplified.OpenAlex.Keywords, :list_keywords},
      url: "/keywords",
      method: :get,
      query: query,
      response: [
        {200, {Amplified.OpenAlex.KeywordsListResponse, :t}},
        {400, {Amplified.OpenAlex.Error, :t}},
        {429, {Amplified.OpenAlex.Error, :t}}
      ],
      opts: opts
    })
  end
end
